...
export default function ChatPage() {
  ...
  const connectWebSocket = useCallback((runId: string) => {
    ...
    ws.onmessage = (event) => {
      try {
        ...
        if (data.type === "node_complete") dispatch({ type: "UPDATE_NODE_STATUS", nodeId: data.node_id, status: "completed" });
        else if (data.type === "node_start") dispatch({ type: "UPDATE_NODE_STATUS", nodeId: data.node_id, status: "running" });
      } catch { /* ignore */ }
    };
  ...
  const handleSend = useCallback(async () => {
    ...
    try {
      const response = await sendChat({ message, auto_execute: false, session_id: state.sessionId || undefined });
      if (response.response_type === "clarification") {
        dispatch({ type: "RECEIVE_CLARIFICATION", message: response.clarification_message, questions: response.questions, sessionId: response.session_id });
      } else {
  ...
  const buildSavePayload = useCallback(() => {
    ...
    return { id: p.pipeline_id, user_intent: p.user_intent, trigger: p.trigger, nodes: p.nodes, edges: p.edges };
  ...
  const handleExecute = useCallback(async () => {
    ...
    try {
      ...
      dispatch({ type: "EXECUTION_COMPLETE", result: { pipeline_id: state.currentPipeline.pipeline_id, run_id: "error", status: "failed", shared_context: {}, node_results: [], errors: ["Execution failed. Check that the backend is running."] } });
    }
  ...
  const handleAddToDashboard = useCallback(async () => {
    if (!state.currentPipeline) return;
    try {
      if (!pipelineSavedRef.current) await savePipeline(buildSavePayload());
  ...
  return (
    <div className="flex flex-col h-screen">
      {showSplitView ? (
        <div className="border-b border-slate-200 px-6 py-3 flex items-center gap-3 flex-shrink-0">
          <Sparkles className="w-4 h-4 text-[#0000FF] flex-shrink-0" />
      ...
      {showSplitView ? (
        <div className="flex-1 flex flex-col overflow-hidden">
          <div className="flex-1 flex overflow-hidden">
            <div className="w-[40%] border-r border-slate-200 flex flex-col overflow-hidden">
              <ThinkingPanel pipeline={state.currentPipeline!} isActive={true} />
            </div>
            <div className="flex-1 flex flex-col overflow-hidden">
              <PipelineGraph nodes={state.currentPipeline!.nodes} edges={state.currentPipeline!.edges} nodeStatuses={state.nodeStatuses} fullHeight />
            </div>
          </div>
          ...
          <div className="border-t border-slate-200 flex-shrink-0">
            {state.phase === "preview" && (
              <div className="px-6 py-3 flex items-center gap-3">
                <button onClick={handleAddToDashboard} className="flex items-center gap-1.5 px-5 py-2 bg-[#0000FF] hover:bg-[#2222FF] text-white text-sm font-medium rounded-lg transition-colors">
                  <LayoutDashboard className="w-3.5 h-3.5" />
                  Add to Dashboard
                </button>
                <button onClick={handleExecute} className="flex items-center gap-1.5 px-4 py-2 border border-slate-300 hover:border-[#0000FF] text-slate-600 hover:text-[#0000FF] text-sm rounded-lg transition-colors">
                  <FlaskConical className="w-3.5 h-3.5" />
                  Test Run
                </button>
                <button onClick={() => dispatch({ type: "REJECT" })} className="flex items-center gap-1.5 px-4 py-2 bg-slate-100 hover:bg-slate-200 text-slate-500 text-sm rounded-lg transition-colors">
                  <X className="w-3.5 h-3.5" />
                  Reject
                </button>
                <span className="text-xs text-slate-400 ml-auto">
                  {state.currentPipeline!.nodes.length} blocks &middot; {state.currentPipeline!.edges.length} edges
                </span>
              </div>
            )}

            {state.phase === "executing" && (
              <div className="px-6 py-3 flex items-center gap-2 text-[#0000FF] text-sm">
            ...
            {executionResult && state.phase === "complete" && (
              <div className="px-6 py-3">
                <div className={`rounded-lg p-4 border ${executionResult.status === "completed" ? "bg-green-50 border-green-200" : "bg-red-50 border-red-200"}`}>
                  <div className="flex items-center gap-2 mb-2">
                    {executionResult.status === "completed" ? <CheckCircle2 className="w-4 h-4 text-green-600" /> : <AlertCircle className="w-4 h-4 text-red-600" />}
                    <span className={`text-sm font-medium ${executionResult.status === "completed" ? "text-green-700" : "text-red-700"}`}>
                      {executionResult.status === "completed" ? "Test run completed" : "Test run failed"}
                    </span>
                  </div>
                  {executionResult.errors.length > 0 && <div className="text-xs text-red-600 mb-2">{executionResult.errors.join(", ")}</div>}
                  {Object.keys(executionResult.shared_context).length > 0 && (
                    <PipelineResultDisplay sharedContext={executionResult.shared_context as Record<string, Record<string, unknown>>} errors={executionResult.errors} />
                  )}
                </div>
                <div className="mt-3 flex items-center gap-3">
                  ...
                  <button onClick={() => dispatch({ type: "RESET" })} className="flex items-center gap-1.5 px-4 py-2 bg-slate-100 hover:bg-slate-200 text-slate-500 text-sm rounded-lg transition-colors">
                    Discard
                  </button>
                </div>
              </div>
              ...
              <div className="px-6 py-3">
                <div className="rounded-lg p-4 border bg-green-50 border-green-200">
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <span className="text-sm font-medium text-green-700">Pipeline added to dashboard</span>
                  </div>
                  <div className="flex items-center gap-3 mt-2">
                    <button onClick={() => router.push("/dashboard")} className="flex items-center gap-1.5 px-4 py-2 bg-[#0000FF] hover:bg-[#2222FF] text-white text-sm font-medium rounded-lg transition-colors">
                      <LayoutDashboard className="w-3.5 h-3.5" />
                      Go to Dashboard
                    </button>
                    <button onClick={() => dispatch({ type: "RESET" })} className="text-xs text-slate-400 hover:text-slate-700 transition-colors">
                      Create another
                    </button>
                  </div>
                </div>
          ...
          <div className="flex-1 overflow-y-auto px-6 py-4 space-y-4">
            {state.messages.length === 0 && (
              <div className="flex flex-col items-center justify-center h-full text-center">
                <Sparkles className="w-12 h-12 text-[#0000FF]/30 mb-4" />
                ...
                <div className="mt-6 flex flex-wrap gap-2 justify-center max-w-lg">
                  {[
                    ...
                    "Buy me protein powder every Tuesday",
                  ].map((example) => (
                    <button
                      key={example}
                      onClick={() => { setInput(example); inputRef.current?.focus(); }}
                      className="text-xs px-3 py-1.5 rounded-full border border-slate-200 text-slate-500 hover:text-[#0000FF] hover:border-[#0000FF] transition-colors"
                    >
                      {example}
                    </button>
                  ))}
                  ...
                  >
                    ...
                    {msg.clarification && (
                      <div className="mt-3 border-t border-slate-200 pt-3">
                        <div className="flex items-center gap-1.5 text-xs text-[#0000FF] mb-2">
                        ...
                        <div className="flex flex-wrap gap-2">
                          {msg.clarification.questions.map((q, i) => (
                            <button
                              ...
                              className="text-xs px-3 py-1.5 rounded-lg border border-[#0000FF]/20 bg-[#0000FF]/5 text-[#0000FF] hover:bg-[#0000FF]/10 hover:border-[#0000FF]/40 transition-colors text-left"
                            >
                              {q}
                            </button>
                          ))}
          ...
          <div className="border-t border-slate-200 p-4 flex-shrink-0">
            <div className="max-w-3xl mx-auto relative">
              <input
                ...
                placeholder={state.phase === "clarifying" ? "Answer the question above..." : "Describe your automation..."}
                disabled={isInputDisabled}
                className="w-full px-4 py-3 pr-12 bg-white border border-slate-300 rounded-xl text-sm text-slate-900 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-[#0000FF]/30 focus:border-[#0000FF] disabled:opacity-50 transition-all"
              />
              <button
                onClick={handleSend}
                disabled={!input.trim() || isInputDisabled}
                className="absolute right-2 top-1/2 -translate-y-1/2 p-2 text-slate-400 hover:text-[#0000FF] disabled:opacity-30 transition-colors"
              >
