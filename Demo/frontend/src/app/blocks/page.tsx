...
export default function BlocksPage() {
  ...
  const [allBlocks, setAllBlocks] = useState<BlockDefinition[]>([]);
  ...
  useEffect(() => {
    if (selectedCategory) {
      setBlocks(allBlocks.filter((b) => b.category === selectedCategory));
    } else {
      setBlocks(allBlocks);
    }
  }, [selectedCategory, allBlocks]);
  ...
  return (
    <div className="p-6">
      ...
      <div className="relative mb-4">
        ...
        {searchQuery && (
          <button
            onClick={() => { setSearchQuery(""); fetchBlocks(); }}
                    ...
                    >
                      <div className="px-4 py-3 space-y-3 text-xs">
                        ...
                        <div>
                          ...
                          <span className="text-gray-400">{(block as any).execution_type || "—"}</span>
                        </div>
                        {(block as any).use_when && (
                          <div>
                            <span className="text-gray-500">Use when:</span>{" "}
                            <span className="text-gray-400">{(block as any).use_when}</span>
                          </div>
                        )}
                        {(block as any).tags && (block as any).tags.length > 0 && (
                          <div>
                            <span className="text-gray-500 block mb-1">Tags:</span>
                            <div className="flex flex-wrap gap-1">
                              {((block as any).tags as string[]).map((tag) => (
                                <span key={tag} className="text-[10px] px-1.5 py-0.5 rounded-full bg-gray-800 text-gray-400 border border-gray-700">
                                  {tag}
                                </span>
                              ))}
                            </div>
